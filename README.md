...
-- cleanup old
pcall(function()
    for _, n in ipairs({"ALIV9Hub","ALIV9Mini","ALIV9Splash"}) do
...
----------------------------------------------------------------
-- إشعار الحقوق – يظهر بعد دقيقتين من تشغيل السكربت
----------------------------------------------------------------
...
local function runSplash()
    ...
    overlay.Size = UDim2.new(1, 0, 1, 0)
    overlay.BackgroundColor3 = Color3.fromRGB(4, 5, 18)
    ...
    card.AnchorPoint = Vector2.new(0.5, 0.5)
    card.Position = UDim2.new(0.5, 0, 0.5, 0)
    card.Size = UDim2.new(0, 380, 0, 190)
    ...
    gradient.Color = ColorSequence.new{
        ColorSequenceKeypoint.new(0, Color3.fromRGB(26, 12, 58)),
        ColorSequenceKeypoint.new(1, Color3.fromRGB(8, 18, 42))
    }
    ...
    logo.Text = "aLkTeBh"
    logo.TextSize = 34
    logo.TextColor3 = Color3.fromRGB(225, 160, 255)
    ...
    sub.Text = "جاري تجهيز الواجهة  •  7aliv9a"
    sub.TextSize = 14
    sub.TextColor3 = Color3.fromRGB(180, 210, 255)

    local bar = Instance.new("Frame", card)
    bar.Position = UDim2.new(0, 30, 1, -42)
    bar.Size = UDim2.new(1, -60, 0, 8)
    bar.BackgroundColor3 = Color3.fromRGB(35, 25, 65)
    bar.BorderSizePixel = 0
    Instance.new("UICorner", bar).CornerRadius = UDim.new(1, 0)
    local fill = Instance.new("Frame", bar)
    fill.Size = UDim2.new(0, 0, 1, 0)
    fill.BackgroundColor3 = Color3.fromRGB(185, 80, 255)
    fill.BorderSizePixel = 0
    Instance.new("UICorner", fill).CornerRadius = UDim.new(1, 0)
    TweenService:Create(fill, TweenInfo.new(1.5, Enum.EasingStyle.Quad), {
...
task.spawn(function() pcall(runSplash) end)

----------------------------------------------------------------
-- Main GUI
----------------------------------------------------------------
local gui = Instance.new("ScreenGui")
gui.Name = "ALIV9Hub"; gui.ResetOnSpawn = false; gui.IgnoreGuiInset = true
...
title.Font = Enum.Font.GothamBlack; title.Text = "aLkTeBh"
...
miniGui.Name = "ALIV9Mini"; miniGui.ResetOnSpawn = false; miniGui.IgnoreGuiInset = true
...
-- Page: نسخ
----------------------------------------------------------------
-- يدعم تحديد أكثر من لاعب، ويتم إرسال الأسماء بهذا الشكل تماماً:
-- playerOne,playerTwo,playerThree (بدون مسافات)
local selectedNames = {}
...
local function getEffectiveName()
    ...
    for _, name in ipairs(selectedNames) do
        if Players:FindFirstChild(name) then
            table.insert(names, nameMode == "three" and name:sub(1, 3) or name)
        end
    end
    ...
    for _, p in ipairs(Players:GetPlayers()) do
        if p ~= LocalPlayer then
            ...
            chip.MouseButton1Click:Connect(function()
                local selectedIndex = table.find(selectedNames, p.Name)
                if selectedIndex then
                    table.remove(selectedNames, selectedIndex)
                else
                    table.insert(selectedNames, p.Name)
                end
                selectedLabel.Text = #selectedNames > 0
                    and ("تم اختيار " .. #selectedNames .. " مستهدف: " .. table.concat(selectedNames, ","))
                    or "اختر لاعب من القائمة"
                for playerName, ch in pairs(playerChips) do
                    local isSelected = table.find(selectedNames, playerName) ~= nil
                    TweenService:Create(ch, TweenInfo.new(0.12), {
                        BackgroundColor3 = isSelected and Color3.fromRGB(0, 120, 60) or Color3.fromRGB(10, 35, 18)
                    }):Play()
                end
            end)
            chip.MouseEnter:Connect(function()
                if not table.find(selectedNames, p.Name) then
            ...
            if table.find(selectedNames, p.Name) then
                chip.BackgroundColor3 = Color3.fromRGB(0, 120, 60)
            end
        end
    end
    selectedLabel.Text = #selectedNames > 0
...
-- إشعار خروج/عودة المستهدف مع استمرار التشغيل على الموجودين
local function isSelectedTarget(name)
    return table.find(selectedNames, name) ~= nil
end
local function showTargetToast(text, color)
    local toastGui = Instance.new("ScreenGui")
    toastGui.Name = "ALIV9TargetToast"
    toastGui.IgnoreGuiInset = true
    toastGui.ResetOnSpawn = false
    toastGui.DisplayOrder = 10002
    toastGui.Parent = guiParent
    local toast = Instance.new("TextLabel", toastGui)
    toast.AnchorPoint = Vector2.new(1, 1)
    toast.Position = UDim2.new(1, -18, 1, -18)
    toast.Size = UDim2.new(0, 300, 0, 42)
    toast.BackgroundColor3 = Color3.fromRGB(12, 14, 30)
    ...
    task.delay(3.2, function()
        if toastGui.Parent then
            ...
            TweenService:Create(toastStroke, TweenInfo.new(0.25), {
                Transparency = 1
            }):Play()
            task.wait(0.3)
...
Players.PlayerRemoving:Connect(function(player)
    if isSelectedTarget(player.Name) then
        local onlineCount = 0
        for _, name in ipairs(selectedNames) do
        ...
        showTargetToast("⚠ خرج: " .. player.Name ..
            "  |  المتصلون: " .. onlineCount, Color3.fromRGB(255, 170, 100))
    end
end)
Players.PlayerAdded:Connect(function(player)
    if isSelectedTarget(player.Name) then
        local onlineCount = 0
        for _, name in ipairs(selectedNames) do
            if Players:FindFirstChild(name) then onlineCount += 1 end
        end
        showTargetToast("↩ عاد: " .. player.Name ..
            "  |  المتصلون: " .. onlineCount, Color3.fromRGB(120, 255, 180))
    end
end)
...
local function startSpam(message)
    ...
    spamThread = task.spawn(function()
        while spamRunning do
            local currentMessage = type(message) == "function" and message() or message
            if currentMessage then
                sendOnce(currentMessage)
            end
            task.wait(0.05)
        end
    end)
...
copySpamBtn.MouseButton1Click:Connect(function()
    ...
    if #selectedNames == 0 then setStatus("اختر لاعباً واحداً أو أكثر أولاً") return end
    local prefix = (prefixBox.Text ~= "" and prefixBox.Text) or ";"
    startSpam(function()
        local name = getEffectiveName()
        return name and buildSpamA(name, (prefixBox.Text ~= "" and prefixBox.Text) or ";") or nil
    end)
...
copyGhostBtn.MouseButton1Click:Connect(function()
    ...
    startSpam(function()
        local name = getEffectiveName()
        return name and buildGhostSpam(name, (prefixBox.Text ~= "" and prefixBox.Text) or ";") or nil
    end)
...
copyLogsBtn.MouseButton1Click:Connect(function()
    ...
    startSpam(function()
        local name = getEffectiveName()
        return name and buildLogsCmd(name, (prefixBox.Text ~= "" and prefixBox.Text) or ";") or nil
    end)
...
copyReBtn.MouseButton1Click:Connect(function()
    ...
    startSpam(function()
        local name = getEffectiveName()
        return name and buildReCmd(name, (prefixBox.Text ~= "" and prefixBox.Text) or ";") or nil
    end)
...
copyPowerBtn.MouseButton1Click:Connect(function()
    ...
    startSpam(function()
        local name = getEffectiveName()
        return name and buildPowerSpam(name, (prefixBox.Text ~= "" and prefixBox.Text) or ";") or nil
    end)
...
local function showBigNotice(text)
    ...
    nGui.Name = "ALIV9Notice"; nGui.ResetOnSpawn = false; nGui.IgnoreGuiInset = true
...
----------------------------------------------------------------
-- Title control (aLkTeBh RGB embedded)
----------------------------------------------------------------
local titleLoaded = false
local ALIV9_RGB_SOURCE = [==[
--[[
    aLkTeBh RGB - Roblox UI Script (Green Theme)
...
pcall(function()
    for _, n in ipairs({"ALIV9RGBHub","ALIV9RGBMini","ALIV9RGBSplash"}) do
        local old = guiParent:FindFirstChild(n)
        if old then old:Destroy() end
        local old2 = PlayerGui:FindFirstChild(n)
        if old2 then old2:Destroy() end
...
local function runSplash()
    local splash = Instance.new("ScreenGui")
    splash.Name = "ALIV9RGBSplash"; splash.ResetOnSpawn = false; splash.IgnoreGuiInset = true
    ...
    n.Font = Enum.Font.GothamBlack; n.Text = "aLkTeBh RGB"
...
----------------------------------------------------------------
local gui = Instance.new("ScreenGui")
gui.Name = "ALIV9RGBHub"; gui.ResetOnSpawn = false; gui.IgnoreGuiInset = true
gui.ZIndexBehavior = Enum.ZIndexBehavior.Sibling; gui.DisplayOrder = 9000
pcall(function() gui.Parent = guiParent end)
if not gui.Parent then gui.Parent = PlayerGui end
...
title.Font = Enum.Font.GothamBlack; title.Text = "⬛ aLkTeBh RGB ⬛"
...
makeSlot(1, "aLkTeBh ON TOP")
makeSlot(2, "NA WAS HERE")
makeSlot(3, "aLkTeBh WAS HERE")

----------------------------------------------------------------
-- Mini draggable bubble
----------------------------------------------------------------
local miniGui = Instance.new("ScreenGui")
miniGui.Name = "ALIV9RGBMini"; miniGui.ResetOnSpawn = false; miniGui.IgnoreGuiInset = true
miniGui.ZIndexBehavior = Enum.ZIndexBehavior.Sibling; miniGui.DisplayOrder = 9001
pcall(function() miniGui.Parent = guiParent end)
if not miniGui.Parent then miniGui.Parent = PlayerGui end
...
do
    ...
    local function ex_getEffectiveName()
        ...
        local names = {}
        for _, name in ipairs(ex_selectedNames) do
            if Players:FindFirstChild(name) then
                table.insert(names, ex_nameMode == "three" and name:sub(1, 3) or name)
            end
        end
        return #names > 0 and table.concat(names, ",") or nil
    end
    ...
    local function ex_refreshPlayers()
        ...
        for _, p in ipairs(Players:GetPlayers()) do
            if p ~= LocalPlayer then
                ...
                chip.MouseButton1Click:Connect(function()
                    local selectedIndex = table.find(ex_selectedNames, p.Name)
                    if selectedIndex then
                        table.remove(ex_selectedNames, selectedIndex)
                    else
                        table.insert(ex_selectedNames, p.Name)
                    end
                    ex_selectedLabel.Text = #ex_selectedNames > 0
                        and ("✔  تم اختيار " .. #ex_selectedNames .. " مستهدف: " .. table.concat(ex_selectedNames, ","))
                        or "⬆ اختر لاعباً أو أكثر من القائمة"
                    for playerName, ch in pairs(ex_playerChips) do
                        local isSelected = table.find(ex_selectedNames, playerName) ~= nil
                        TweenService:Create(ch, TweenInfo.new(0.12), {
                            BackgroundColor3 = isSelected and Color3.fromRGB(0, 130, 60) or ex_DARK2,
                            TextColor3 = isSelected and ex_YELLOW or ex_WHITE
                        }):Play()
                    end
                end)
                chip.MouseEnter:Connect(function()
                    if not table.find(ex_selectedNames, p.Name) then
                ...
                if table.find(ex_selectedNames, p.Name) then
                    chip.BackgroundColor3 = Color3.fromRGB(0, 130, 60)
                    chip.TextColor3 = ex_YELLOW
                end
            end
        end
        ex_selectedLabel.Text = #ex_selectedNames > 0
    ...
    Players.PlayerRemoving:Connect(function(player)
        if table.find(ex_selectedNames, player.Name) then
            local onlineCount = 0
            for _, name in ipairs(ex_selectedNames) do
                if name ~= player.Name and Players:FindFirstChild(name) then
                    onlineCount += 1
    ...
    local function ex_buildCustomMsg(input, name, prefix)
        ...
        local block
        if hasCmd and not name then
            return nil
        elseif hasCmd and name then
            local parts = {}
            for _, t in ipairs(tokens) do
    ...
    local function ex_startSpam()
        ...
        if hasCmd0 and not ex_getEffectiveName() then
            ex_setStatus("⚠ اختر لاعباً أو أكثر من القائمة أولاً"); return
        end
        ex_spamRunning = true
